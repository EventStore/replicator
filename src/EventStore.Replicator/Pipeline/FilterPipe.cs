...
namespace EventStore.Replicator.Pipeline {
    public class FilterContext : BasePipeContext, PipeContext {
        public FilterContext(OriginalEvent originalEvent, CancellationToken cancellationToken) : base(cancellationToken) => OriginalEvent = originalEvent;
    ...
    public class EventFilterSpecification : IPipeSpecification<FilterContext> {
        ...
        public void Apply(IPipeBuilder<FilterContext> builder) => builder.AddFilter(new EventFilterFilter(_filter));

        public IEnumerable<ValidationResult> Validate() {
            if (_filter == null) yield return this.Failure("validationFilterPipe", "Event filter is missing");
        }
    }

    public static class EventFilterPipeExtensions {
        public static void UseEventFilter(this IPipeConfigurator<FilterContext> configurator, FilterEvent filter)
